...
def set_catalog(ocp_version: str, operators: List[Dict]) -> None:
    ...
    with _connect() as conn:
        conn.execute(
            "INSERT OR REPLACE INTO catalog_cache (ocp_version, cached_at, operators) VALUES (?, ?, ?)",
            (ocp_version, datetime.now().isoformat(), json.dumps(operators, ensure_ascii=False)),
        )
    ...
    with _connect() as conn:
        conn.execute(
            """INSERT OR REPLACE INTO package_cache
               (ocp_version, package_name, cached_at, channels)
               VALUES (?, ?, ?, ?)""",
            (
                ocp_version,
                package_name,
